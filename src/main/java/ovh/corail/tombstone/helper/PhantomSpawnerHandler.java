...
public class PhantomSpawnerHandler {
    ...
    public void checkWorld(ServerWorld world) {
        ...
        Predicate<ISpecialSpawner> predic = spawner -> spawner instanceof PhantomSpawner;
        if (world.field_241104_N_.stream().noneMatch(predic)) {
            return;
        }
        ImmutableList.Builder<ISpecialSpawner> builder = new ImmutableList.Builder<>();
        for (ISpecialSpawner spawner : world.field_241104_N_) {
            builder.add(predic.test(spawner) ? new CustomPhantomSpawner() : spawner);
        }
